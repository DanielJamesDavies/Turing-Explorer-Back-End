...
def get_latent_data_stream(layer, latent, latent_data_path, latent_data_dir_list):
    ...
    yield json.dumps({ 'message': 'Success', 'first': True, 'final': False, 'latentFrequency': latent_frequency, 'topSequencesList': None, "postFromSequenceLatentData": None }) + "<|END_OF_RESPONSE_CHUNK_12|>"
    ...
    yield json.dumps({ 'message': 'Success', 'first': False, 'final': False, 'latentFrequency': latent_frequency, 'topSequencesList': top_sequences_list[:24], "postFromSequenceLatentData": None }) + "<|END_OF_RESPONSE_CHUNK_12|>"
    ...
    for post_from_sequence_latent_data in post_from_sequence_latent_data_responses:
        yield json.dumps({ 'message': 'Success', 'first': True, 'final': post_from_sequence_latent_data["final"], 'latentFrequency': latent_frequency, 'topSequencesList': top_sequences_list[:24] if post_from_sequence_latent_data["layerUnembedTokenFrequencies"] is not None else None, "postFromSequenceLatentData": post_from_sequence_latent_data }) + "<|END_OF_RESPONSE_CHUNK_12|>"
...
def get_post_from_sequence_latent_data(layer, latent, latent_data_path, latent_data_dir_list, tokenizer):
    ...
    start_time_get_top_other_latents = time.time()
    ...
    del latents_sae_frequencies
    print(f"Get Latent Data  |  Got Top Other Latents  |  Duration: {time.time()-start_time_get_top_other_latents:.2f}s")
